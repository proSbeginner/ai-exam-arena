...
export function Admin() {
  ...
  return (
    <>
      <Toast
        key={admin.error ?? "admin-error-none"}
        message={admin.error}
        variant="error"
      />
      <main className="min-h-screen bg-gradient-to-br from-pink-100 via-purple-100 to-blue-100 p-4 sm:p-8">
        <section className="mx-auto w-full max-w-3xl">
          ...
          <AdminQuestionForm
            ...
            onCorrectOptionChange={(id) =>
              admin.updateForm("correctOptionId", id)
            }
            onRemoveLabel={admin.removeLabel}
            onRemoveOption={(index) =>
              admin.updateForm(
                "options",
                admin.form.options.filter(
          ...
          <AdminQuestionList
            ...
            onLoad={(limit) =>
              void admin.loadQuestions(admin.ADMIN_LOAD_DELAY_MS, limit)
            }
